...
public class Driver {
    ...
    public static Duration setImplicitlyWait(int seconds) {
        return setImplicitlyWait(Duration.ofSeconds(seconds));
    }

    @Step("Set Implicitly Wait")
    public static Duration setImplicitlyWait(Duration duration) {
        Log.pass("### SET IMPLICITLY WAIT: " + duration.getSeconds() + " seconds");
        var currentWait = Driver.getDriver().manage().timeouts().getImplicitWaitTimeout();
